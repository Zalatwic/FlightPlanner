        ...
        public List<SeatConfiguration> GetSeatConfigurations() {
            ...
            foreach (Match currentMatch in configurationInfo) {
                ...
                foreach (PassengerType type in Enum.GetValues(typeof(PassengerType))) {
                    string[] currentValue = currentGroup[(int)type + 2].Value.Split("<br/>");

                    // String split successfully.
                    if (currentGroup[(int)type + 2].Value != currentValue[0]) {
                        CabinObject currentCabin = new CabinObject(
                            type,
                            (SeatType)Enum.Parse(typeof(SeatType), currentValue[1].Replace("-", "").Replace(" ", "_")),
                            int.Parse(currentValue[0]),
                            int.Parse(currentGroup[(int)type + 6].Value));
                    }
                }

                // Build the current ratings, currentGroup[9] is the configuration ID.
                currentHTML = currentInst.ReadWebsite("app/seating/editor/" + currentGroup[10].Value + "?50-1.0-editor~form-editor-deck~ratings~tab-link");
                ...
                foreach (RatingsDistance distance in Enum.GetValues(typeof(RatingsDistance))) {
                    ...
                    currentConfiguration.ChangeRating(PassengerType.Economy, distance, int.Parse(currentRatingMatches[1].Value));
                    currentConfiguration.ChangeRating(PassengerType.Business, distance, int.Parse(currentRatingMatches[2].Value));
                    currentConfiguration.ChangeRating(PassengerType.First, distance, int.Parse(currentRatingMatches[3].Value));
                }

                // Get the ID of the current plane type.
                currentRegex = String.Format(Expressions.GetAircraftID, currentDistance);
            ...
            for (int x = 0; x < fleetNames.Count; x++) {
                ...
                foreach (Match plane in currentPlanes) {
                    ...
                    currentFleet.AddAircraft(new OwnedAircraft(
                        int.Parse(currentPlaneMatches[1].Value),
                        currentPlaneMatches[2].Value,
                        currentPlaneMatches[3].Value,
                        int.Parse(currentPlaneMatches[4].Value),
                        currentPlaneMatches[5].Value,
                        currentPlaneMatches[6].Value,
                        double.Parse(currentPlaneMatches[7].Value),
                        double.Parse(currentPlaneMatches[8].Value),
                        double.Parse(currentPlaneMatches[9].Value),
                        Manwell.FindSeatConfiguration(currentConfigurations, int.Parse(currentPlaneMatches[4].Value), int.Parse(currentPlaneMatches[11].Value), int.Parse(currentPlaneMatches[12].Value), int.Parse(currentPlaneMatches[13].Value))
                        ));
                }
